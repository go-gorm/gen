...
func init() {
	InitializeDB()
	err := _gen_test_db.AutoMigrate(&tests_test.User{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&tests_test.User{}) fail: %s", err)
	}
}
...
func Test_userQuery(t *testing.T) {
	...
	err = _do.Create(&tests_test.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.Save(&tests_test.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.CreateInBatches([]*tests_test.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*tests_test.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&tests_test.User{}, 0, 1)
	if err != nil {
